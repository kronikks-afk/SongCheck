...
- The model used server-side is `gemini-2.5-flash`. If Google retires
  or renames it, swap the model string in `api/mix-feedback.js` and
  `api/lyrics.js` — check https://ai.google.dev/gemini-api/docs/models
  for the current free-tier model list.
